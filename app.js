...
function renderWeekCalendar(){
  ...
  document.getElementById("calendar-content").innerHTML=`<div class="week-calendar">${names.map((name,index)=>{const d=new Date(start);d.setDate(d.getDate()+index);const n=new Date(d);n.setDate(n.getDate()+1);const list=items.filter(i=>overlaps(i,d,n)),isToday=localDateTime(d).slice(0,10)===localDateTime(new Date()).slice(0,10);return `<div class="calendar-day ${isToday?"today":""}"><div class="calendar-day-head"><span>${name}${isToday?`<b class="today-tag">I dag</b>`:""}</span><strong>${d.getDate()}</strong></div><div class="day-events">${list.map(i=>calendarEvent(i,d,n)).join("")}</div></div>`;}).join("")}</div>`;
  requestAnimationFrame(()=>requestAnimationFrame(()=>centerTodayInMobileCalendar("auto")));
}
...
function renderMonthCalendar(){
  ...
  document.getElementById("calendar-content").innerHTML=`<div class="month-calendar">${names.map(n=>`<div class="month-weekday">${n}</div>`).join("")}${Array.from({length:42},(_,index)=>{const d=new Date(gridStart);d.setDate(d.getDate()+index);const n=new Date(d);n.setDate(n.getDate()+1);const list=items.filter(i=>overlaps(i,d,n)),dayKey=localDateTime(d).slice(0,10),dayLabel=formatDate(d,{weekday:"long",day:"numeric",month:"long"}),isToday=dayKey===localDateTime(new Date()).slice(0,10);return `<div class="month-day ${d.getMonth()!==first.getMonth()?"outside":""} ${isToday?"today":""}"><button class="month-day-trigger" data-open-day="${dayKey}" aria-label="Åpne ${esc(dayLabel)}"><strong>${d.getDate()}</strong><span class="month-day-meta">${isToday?`<b class="today-tag">I dag</b>`:""}${list.length?`<i>${list.length}</i>`:""}</span></button><div class="month-day-events">${list.slice(0,4).map(i=>{const subject=i.subject?subjectById(i.subject):null;const label=subject?subject.code:i.title;const room=i.location?` · ${i.location}`:"",start=toDate(i.date),end=eventEnd(i),time=start<d?(i.endDate&&end<=n?`til ${formatTime(i.endDate)}`:"hele dagen"):(i.endDate&&end>n?`${formatTime(i.date)} →`:formatTime(i.date));return `<button class="month-event type-${i.type}" data-view-event="${esc(i.id)}" title="Åpne detaljer for ${esc(label)}">${esc(time)} ${esc(label+room)}</button>`;}).join("")}${list.length>4?`<small>+${list.length-4} flere</small>`:""}</div></div>`;}).join("")}</div>`;
}
function openDayDetail(dayKey){
  const start=new Date(`${dayKey}T00:00`),end=new Date(start);end.setDate(end.getDate()+1);
  const items=filteredCalendarItems(start,end),dialog=document.getElementById("day-detail-dialog");
  document.getElementById("day-detail-title").textContent=formatDate(start,{weekday:"long",day:"numeric",month:"long",year:"numeric"});
  document.getElementById("day-detail-events").innerHTML=items.length?items.map(item=>{const subject=item.subject?subjectById(item.subject):null,category=categoryOf(item),startsBefore=toDate(item.date)<start,time=startsBefore?(item.endDate?`Pågår til ${formatTime(item.endDate)}`:"Pågår"):formatRange(item);return `<button class="day-detail-event type-${item.type}" data-view-event="${esc(item.id)}"><span class="day-detail-icon">${ICONS[category]||"·"}</span><span><small>${esc(subject?subject.code:LABELS[category])}</small><strong>${esc(item.title)}</strong><em>${esc(time)}${item.location?` · ${esc(item.location)}`:""}</em></span><b>→</b></button>`;}).join(""):empty("Ingen hendelser denne dagen med valgte filtre.");
  dialog.showModal();bindDynamicActions();
}
...
function renderSubjectDetail(id){
  ...
  const openTodos=todos.filter(t=>!t.done).length,nextClass=classes[0],nextAssessment=assessments[0];
  const tabs=[{id:"todos",label:"Gjøremål",count:openTodos},{id:"classes",label:"Forelesninger",count:classes.length},{id:"assessments",label:"Frister og eksamen",count:assessments.length}];
  document.getElementById("subject-detail-content").innerHTML=`<article class="subject-hero card" style="--subject-color:${esc(subject.color||"#4077c8")}"><div><p class="eyebrow">${esc(subject.code)}</p><h2>${esc(subject.name)}</h2><p class="muted">Alt for emnet samlet og sortert på ett sted.</p></div><button class="primary" data-add-subject-todo="${esc(id)}">+ Nytt gjøremål</button><div class="subject-snapshot"><span><small>Åpne gjøremål</small><strong>${openTodos}</strong></span><span><small>Neste forelesning</small><strong>${nextClass?formatDate(nextClass.date,{day:"numeric",month:"short"}):"Ingen"}</strong></span><span><small>Neste frist</small><strong>${nextAssessment?formatDate(nextAssessment.date,{day:"numeric",month:"short"}):"Ingen"}</strong></span></div></article><div class="subject-tabs" role="tablist" aria-label="Innhold for ${esc(subject.code)}">${tabs.map(tab=>`<button role="tab" aria-selected="${subjectDetailTab===tab.id}" class="${subjectDetailTab===tab.id?"active":""}" data-subject-tab="${tab.id}">${tab.label}<span>${tab.count}</span></button>`).join("")}</div><div class="subject-tab-panels"><article class="card subject-panel ${subjectDetailTab==="todos"?"active":""}" data-subject-panel="todos"><div class="card-head"><div><p class="eyebrow">Arbeidsliste</p><h2>Gjøremål</h2></div></div><div class="todo-list">${todos.length?todos.map(t=>todoRow(t)).join(""):empty("Ingen gjøremål for dette emnet.")}</div></article><article class="card subject-panel ${subjectDetailTab==="classes"?"active":""}" data-subject-panel="classes"><div class="card-head"><div><p class="eyebrow">Plan</p><h2>Forelesninger</h2></div></div><div class="event-list">${classes.length?classes.map(i=>eventRow(i,false)).join(""):empty("Ingen kommende forelesninger.")}</div></article><article class="card subject-panel ${subjectDetailTab==="assessments"?"active":""}" data-subject-panel="assessments"><div class="card-head"><div><p class="eyebrow">Vurdering</p><h2>Frister og eksamen</h2></div></div><div class="event-list">${assessments.length?assessments.map(i=>eventRow(i)).join(""):empty("Ingen kommende frister.")}</div></article></div>`;
}
function renderWork(){
...
function bindDynamicActions(){
  ...
  document.querySelectorAll("[data-view-event]").forEach(btn=>btn.onclick=()=>{const dayDialog=document.getElementById("day-detail-dialog");if(dayDialog.open)dayDialog.close();openEventDetail(btn.dataset.viewEvent);});
  document.querySelectorAll("[data-open-day]").forEach(btn=>btn.onclick=()=>openDayDetail(btn.dataset.openDay));
...
function switchView(id){document.querySelectorAll(".view").forEach(v=>v.classList.toggle("active",v.id===id));document.querySelectorAll(".nav-btn[data-view]").forEach(b=>b.classList.toggle("active",b.dataset.view===id));document.getElementById("page-title").textContent=VIEW_TITLES[id]||"Min hverdag";closeMobileMenu();window.scrollTo({top:0,behavior:"smooth"});if(id==="calendar"){if(calendarMode==="week"){calendarDate=startOfWeek(new Date());renderCalendar();bindDynamicActions();}setTimeout(()=>centerTodayInMobileCalendar("auto"),80);setTimeout(()=>centerTodayInMobileCalendar(),260);}}
...
function openSettings(){closeMobileMenu();document.getElementById("hourly-wage").value=data.settings.hourlyWage||"";window.cloudDashboard?.showConfiguration();document.getElementById("settings-dialog").showModal();}
...
document.getElementById("import-data-file").onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const imported=JSON.parse(await file.text());if(!imported||!Array.isArray(imported.items)||!Array.isArray(imported.todos))throw new Error("Ugyldig datafil");const mergeById=(current,incoming)=>[...new Map([...current,...incoming].map(item=>[item.id,item])).values()];const merged={...data,...imported,subjects:mergeById(data.subjects,Array.isArray(imported.subjects)?imported.subjects:[]),items:mergeById(data.items,imported.items),todos:mergeById(data.todos,imported.todos),settings:{...data.settings,...imported.settings}};window.applyCloudDashboard(merged);persist();document.getElementById("settings-dialog").close();toast("Dataene ble slått sammen og synkroniseres");}catch(error){toast(`Import feilet: ${error.message}`);}finally{e.target.value="";}};
...
document.querySelectorAll("[data-calendar-mode]").forEach(btn=>btn.onclick=()=>{calendarMode=btn.dataset.calendarMode;calendarDate=calendarMode==="week"?startOfWeek(new Date()):new Date(calendarDate.getFullYear(),calendarDate.getMonth(),1);renderCalendar();bindDynamicActions();if(calendarMode==="week"){setTimeout(()=>centerTodayInMobileCalendar("auto"),60);setTimeout(()=>centerTodayInMobileCalendar(),220);}});
